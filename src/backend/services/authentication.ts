...
import { always, assoc, dissoc, isNil, omit, pipe, propEq } from 'ramda'
...
const refreshToken = (user: WithId<User>): Promise<Token> => {
  const token = sign({ id: user._id, email: user.email, name: user.name }, `${config.TOKEN_KEY}`, {
    expiresIn: '90d'
  })
...
const register = endpoint(
  { register: body(TRegister), users: coll('users') },
  async ({ register, users }) => {
    ...
    return users.insertOne({ ...data }).then(x => refreshToken)
  }
)
